...
// >>> Envia token em dois headers e com credenciais
async function authFetch(input, init = {}) {
  let { data: { session } } = await supabase.auth.getSession();
  if (!session) {
    // tenta atualizar (se expirou)
...
async function carregarLista(){
  const resp = await authFetch(`/.netlify/functions/adminUsers?page=${state.page}&size=${state.size}`);
  if(!resp.ok){
...
async function criar(){
  ...
  const resp = await authFetch("/.netlify/functions/adminUsers",{ method:"POST", body: JSON.stringify(payload) });
  const json = await resp.json();
...
async function atualizarSelecionado(upd){
  ...
  const resp = await authFetch("/.netlify/functions/adminUsers",{ method:"PUT", body: JSON.stringify(upd) });
  const json = await resp.json();
...
async function excluir(){
  ...
  const resp = await authFetch("/.netlify/functions/adminUsers",{ method:"DELETE", body: JSON.stringify({ id: state.selected.id }) });
  const json = await resp.json();
...
async function enviarLinkRedefinicao(){
  ...
  const resp = await authFetch("/.netlify/functions/adminUsers?action=reset",{ method:"POST", body: JSON.stringify({ email }) });
  const json = await resp.json();
